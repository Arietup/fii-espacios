    ...
    <AdminModal title={`Editar: ${estado.nombre}`} backHref="/admin/estados" backLabel="Estados">
      <form action={updateEstado.bind(null, estado.id)} className="grid gap-5 sm:grid-cols-2">
        <label className="space-y-1">
          ...
          <input name="nombre" required defaultValue={estado.nombre} className="h-10 w-full rounded-lg border border-[var(--border-soft)] px-3 text-sm" />
        </label>
        <label className="space-y-1">
          <span className="text-sm font-medium text-[var(--text)]">Color</span>
          <input name="color" defaultValue={estado.color} className="h-10 w-full rounded-lg border border-[var(--border-soft)] px-3 text-sm" />
        </label>
        <label className="space-y-1">
          <span className="text-sm font-medium text-[var(--text)]">Orden</span>
          <input name="orden" type="number" defaultValue={estado.orden} className="h-10 w-full rounded-lg border border-[var(--border-soft)] px-3 text-sm" />
        </label>
        <label className="space-y-1">
          <span className="text-sm font-medium text-[var(--text)]">Estado</span>
          <select name="activo" defaultValue={estado.activo.toString()} className="h-10 w-full rounded-lg border border-[var(--border-soft)] px-3 text-sm">
        ...
        <label className="space-y-1 sm:col-span-2">
          ...
          <textarea name="descripcion" rows={3} defaultValue={estado.descripcion ?? ""} className="w-full rounded-lg border border-[var(--border-soft)] px-3 py-2 text-sm" />
        </label>
